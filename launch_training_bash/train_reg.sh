...
# ---- Parameters ----
EXPERIMENT=training_sweeps/RegLambda
...
CODE_REG_LAMBDAS=(1e-3 1e-4 5e-4 1e-5 5e-5 1e-6)

for codereg in "${CODE_REG_LAMBDAS[@]}"; do
    # for anneal in "${ANNEAL[@]}"; do    
...
BIPBOP
        ...
        while true; do
            ...
            if (( free > MEM_REQUIRED_MB + SAFETY_MARGIN_MB && running < MAX_PARALLEL )); then
                ...
                echo "Free GPU memory: ${free}MB | Running jobs: ${running} | Launching code reg = ${codereg} ..."

                logfile="$LOG_DIR/Reg${codereg}.log"
            ...
            echo "Waiting for free GPU memory or jobs ending: $(date)"
