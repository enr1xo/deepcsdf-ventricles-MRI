...
def extract_raw_ventricle_surfaces(mesh, tags_metadata = VENTRICLE_TAGS_METADATA):
    ...
    # #  Extract surfaces
    # # =============================================================== #
    whole_surface = mesh.extract_surface(algorithm="dataset_surface")
    ...
    epicardium_surface = surf.extract_surface(algorithm="dataset_surface")

    # ===== right venticle endocardium ===== #
    surf = whole_surface.extract_cells( np.isin( surface_elemTags, RV_endo_tags) )
    surf = surf.connectivity(extraction_mode = 'largest')
    RV_endo_surface = surf.extract_surface(algorithm="dataset_surface")
    ...
    LV_endo_surface = surf.extract_surface(algorithm="dataset_surface")
...
def extract_closed_ventricle_surfaces(mesh, tags_metadata = VENTRICLE_TAGS_METADATA):
    ...
    # boundary_edges = LV_endo_surface.extract_feature_edges(
    #     boundary_edges=True,
    #     feature_edges=False,
    ...
    #     logger.warning(
    #         f"LV still has "
    #         f"{boundary_edges.n_cells} boundary edges. "
    #         f"Applying second repair."
    #     )

    #     LV_endo_surface = make_surface_watertight(
    #         LV_endo_surface
    #     )

    #     LV_endo_surface.field_data[
    #         "second_repair_applied"
    ...
    #     logger.error(
    #         "LV mesh is not watertight"
    #     )

    # ===== left ventricle endocardium ===== #

    surf = whole_surface.extract_cells(
        np.isin(surface_elemTags, LV_endo_tags)
    )

    surf = surf.connectivity(
    ...
    # boundary_edges = RV_endo_surface.extract_feature_edges(
    #     boundary_edges=True,
    #     feature_edges=False,
    #     manifold_edges=False,
    #     non_manifold_edges=False,
    ...
    #     RV_endo_surface = make_surface_watertight(
    #         RV_endo_surface
    #     )

    #     RV_endo_surface.field_data[
    #         "second_repair_applied"
    #     ] = np.array([1], dtype=np.int8)
    ...
    #     logger.error(
    #         "RV mesh is not watertight"
    #     )
    
    # ===== right ventricle endocardium ===== #

    surf = whole_surface.extract_cells(
        np.isin(surface_elemTags, RV_endo_tags)
    )
...
def extract_processed_ventricle_surfaces(patient_name, reference_name, reference_mesh, source_dir = PATIENT_MESHES_DIR):
    ...
    if patient_name != reference_name: # skip alignment for reference mesh
        ...
        _, R, t = align_to_reference_mesh(
            source_mesh = original_scaled.extract_surface(algorithm="dataset_surface"),
            target_mesh = reference_mesh_scaled.extract_surface(algorithm="dataset_surface"),
