...
def find_pointcloud_noise(
    ...
    if active_cols is None:
        active_cols = torch.arange(sdf_gt.shape[1], device=sdf_gt.device)
    ...
    for _ in range(max_iter):
        ...
        for i in range(num_epochs_fit_latent):
            ...
            recon_loss = loss_fn(
                sdf_pred[:, active_cols],
                sdf_gt[:, active_cols],
            )
            chunk_loss = recon_loss / (num_samp_per_scene * len(active_cols))
...
def surface_specific_sdf_mask(
    ...
    # MRI-like options
    use_mri_slices: bool = False,
    mri_epsilon_mm: float = 0.1,
    mri_delta_mm: float = 6.0,
    scale_to_original_um: float | None = None,
):
    """
    Builds a boolean mask selecting:
        1) points near chosen surfaces in SDF sense
        2) optionally also inside MRI-like slices
    """

    if sdf_gt.shape[0] != 5000:
        raise ValueError(f"Expected 5000 rows, got {sdf_gt.shape[0]}")

    if use_mri_slices and coords_gt is None:
        raise ValueError("coords_gt required when use_mri_slices=True")

    if use_mri_slices and scale_to_original_um is None:
        raise ValueError(
            "scale_to_original_um required when use_mri_slices=True"
        )
    ...
    mask = torch.zeros(
        sdf_gt.shape[0],
        dtype=torch.bool,
        device=sdf_gt.device,
    )

    surface_ranges = {
        "epi": (slice(0, 1500), 0),
        "lv":  (slice(1500, 3250), 1),
        "rv":  (slice(3250, 5000), 2),
    }

    for surface_name in surfaces:

        sl, sdf_col = surface_ranges[surface_name]

        mask[sl] = (
            torch.abs(sdf_gt[sl, sdf_col]) <= threshold
        )

    return mask

def combined_sdf_percentage_surface_mask(
    sdf_gt,
    threshold: float = 0.005,
    surfaces=("epi", "lv", "rv"),
    ...
    for surface_name in surfaces:
        ...
        surface_indices = torch.arange(
            sl.start,
            sl.stop,
            device=sdf_gt.device,
        )

        sdf_mask = torch.abs(sdf_gt[sl, sdf_col]) <= threshold
        filtered_indices = surface_indices[sdf_mask]

        n_keep = int(len(filtered_indices) * percentage)

        if n_keep == 0:
            continue

        chosen_np = rng.choice(
            filtered_indices.cpu().numpy(),
            size=n_keep,
            replace=False,
        )

        chosen = torch.from_numpy(chosen_np).to(sdf_gt.device)

        final_mask[chosen] = True

        print(
            f"{surface_name}: "
            f"SDF kept {len(filtered_indices)} / {sl.stop - sl.start}, "
            f"percentage kept {n_keep}"
        )
...
def mri_like_mask(
    ...
    epsilon = epsilon_um / scale_to_original_um
    delta = delta_um / scale_to_original_um
...
def run(
    ...
    single_patient_name: str | None = None,

    subsampling_test: bool = False,

    sampling_mode: str = "none",

    sdf_subsampling_threshold: float = 0.005,
    sdf_filter_surfaces: tuple[str, ...] = ("epi", "lv", "rv"),

    combined_subsample_fraction: float = 0.25,
    combined_subsample_seed: int = 42,  
    ...
    shape_indices = list(range(len(dataset)))

    if single_patient_name is not None:
        matches = [
            i for i, name in enumerate(patient_names)
            if name.lower() == single_patient_name.lower()
        ]

        if len(matches) == 0:
            raise ValueError(
                f"Patient '{single_patient_name}' not found. "
                f"Available patients include: {patient_names[:10]}"
            )

        if len(matches) > 1:
            raise ValueError(f"Multiple matches found for patient '{single_patient_name}'")

        shape_indices = matches
    ...
    for shape_idx in shape_indices:
        ...
        if subsampling_test:
            ...
            data_full = np.load(coords_and_sdf_file)

            xyz_gt = torch.from_numpy(data_full[:, :3]).float()
            sdf_gt = torch.from_numpy(data_full[:, 3:]).float()
            ...
            if sampling_mode == "none":
                ...
                print("\nNo subsampling filter")

            # =================================================
            # SDF FILTER
            # =================================================

            elif sampling_mode == "sdf":

                mask = surface_specific_sdf_mask(
                    sdf_gt=sdf_gt,
                    threshold=sdf_subsampling_threshold,
                    surfaces=sdf_filter_surfaces,
                )

                print("\nUsing SDF subsampling")

            elif sampling_mode == "combined":

                mask = combined_sdf_percentage_surface_mask(
                    sdf_gt=sdf_gt,
                    threshold=sdf_subsampling_threshold,
                    surfaces=sdf_filter_surfaces,
                    percentage=combined_subsample_fraction,
                    seed=combined_subsample_seed,
                )

                print("\nUsing combined subsampling: SDF + percentage + surface")

            # =================================================
            # MRI FILTER
            # =================================================

            elif sampling_mode == "mri":

                patient_dir = patient_meshes_dir / patient_name

                mesh_file = next(
                    patient_dir.rglob("*epicardium-processed.vtp"),
                    None
                )

                if mesh_file is None:
                    raise FileNotFoundError(
                        f"No mesh found for {patient_name}"
                    )

                mesh = pv.read(mesh_file)

                scale_to_original_um = (
                    mesh.field_data["scale-tooriginalrange"][0]
                )

                mask = mri_like_mask(
                    coords_gt=xyz_gt,
                    scale_to_original_um=scale_to_original_um,
                    epsilon_mm=mri_epsilon_mm,
                    delta_mm=mri_delta_mm,
                )

                print("\nUsing MRI-like subsampling")
            ...
            print(f"before = {xyz_gt.shape[0]} points")
            print(f"after  = {mask.sum().item()} points")
            ...
            if reconstruct_from == "la":
                near_la = torch.abs(sdf_gt[:, 1]) <= 0.005
        ...
        if subsampling_test:
            active_cols = torch.tensor(
                [surface_cols[s] for s in sdf_filter_surfaces],
                device=DEVICE,
                dtype=torch.long,
            )
        else:
            active_cols = torch.arange(decoder.out_dim, device=DEVICE)
        ...
        for _ in tqdm(range(num_epochs)):
            ...
            chunk_loss = loss_fn(
                                sdf_pred[:, active_cols],
                                sdf_gt[:, active_cols],
                            ) / (num_samp_per_scene * len(active_cols))
...
def parse_args():
    ...
    parser.add_argument("--experiment_name", "-e", type=str, default="architecture_exploration")
    ...
    parser.add_argument("--patient_name", "-p", type=str, default=None)
    ...
    #=======
    parser.add_argument(
        "--combined_subsample_fraction",
        type=float,
        default=0.25,
    )

    parser.add_argument(
        "--combined_subsample_seed",
        type=int,
        default=42,
    )
...
def main():
    ...
    kwargs = {
        ...
        "single_patient_name": args.patient_name,
        "subsampling_test": args.subsampling_test,

        "sampling_mode": args.sampling_mode,

        "sdf_subsampling_threshold": args.sdf_subsampling_threshold,
        "sdf_filter_surfaces": tuple(args.sdf_filter_surfaces),     

        "combined_subsample_fraction": args.combined_subsample_fraction,
        "combined_subsample_seed": args.combined_subsample_seed,

        "mri_epsilon_mm": args.mri_epsilon_mm,
        "mri_delta_mm": args.mri_delta_mm,
    }
